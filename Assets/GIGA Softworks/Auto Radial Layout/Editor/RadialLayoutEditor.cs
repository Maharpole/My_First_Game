...
namespace GIGA.AutoRadialLayout.Editor
{
	...
	public class RadialLayoutEditor : UnityEditor.Editor
	{
		...
		public void OnSceneGUI()
		{
			...
			float canvasScale = t.Canvas.transform.localScale.x;
		...
		private void DrawResizingHandles(RadialLayout layout)
		{
			...
			size *= layout.Canvas.transform.localScale.x;

			Vector3 snap = layout.handle_snap ? Vector3.one * layout.handle_snap_resolution : Vector3.zero;
			Color exColor = Handles.color;

			Vector3 prevHandlePos = layout.transform.position + layout.circleRadius * layout.Canvas.transform.localScale.x * (Vector3.left * Mathf.Sin((layout.transform.eulerAngles.z + layout.rotationOffset) * Mathf.Deg2Rad) + Vector3.up * Mathf.Cos(-(layout.transform.eulerAngles.z + layout.rotationOffset) * Mathf.Deg2Rad));
			...
			if (newHandlePos != prevHandlePos)
			{
				...
				if (!layout.handle_lockRadius)
					layout.circleRadius = radius / layout.Canvas.transform.localScale.x;
